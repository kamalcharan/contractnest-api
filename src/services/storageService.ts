  ...
  try {
    ...
    // Only add page param if it's provided
    if (page !== undefined) {
    ...
    const response = await axios.get(
      `${getSupabaseApiUrl()}/functions/v1/tenant-storage/files${params.toString() ? '?' + params : ''}`,
      {
        headers: {
          'Authorization': authToken,
          'x-tenant-id': tenantId,
    ...
    // Return empty array for other errors too to prevent UI issues
    return [];
  }
},
